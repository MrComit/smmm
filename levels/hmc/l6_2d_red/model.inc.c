Gfx l6_2d_red__2d_red_coin_ci4_aligner[] = {gsSPEndDisplayList()};
u8 l6_2d_red__2d_red_coin_ci4[] = {
	, 
	
};

Gfx l6_2d_red__2d_red_coin_ci4_pal_rgba16_aligner[] = {gsSPEndDisplayList()};
u8 l6_2d_red__2d_red_coin_ci4_pal_rgba16[] = {
	0x00, 0x00, 0x58, 0x43, 0x78, 0x43, 0xa8, 0x85, 
	0x18, 0x01, 0x68, 0x43, 0xb0, 0x85, 0xc0, 0x85, 
	0xe0, 0xc7, 0xa0, 0x85, 0x28, 0x01, 0x90, 0x85, 
	0x30, 0x43, 0x88, 0x85, 0x48, 0x43, 0x50, 0x43, 
	
};

Vtx l6_2d_red_2DRedCoin_mesh_layer_4_vtx_cull[8] = {
	{{ {-35, -35, 0}, 0, {-16, -16}, {0, 0, 0, 0} }},
	{{ {-35, 35, 0}, 0, {-16, -16}, {0, 0, 0, 0} }},
	{{ {-35, 35, 0}, 0, {-16, -16}, {0, 0, 0, 0} }},
	{{ {-35, -35, 0}, 0, {-16, -16}, {0, 0, 0, 0} }},
	{{ {35, -35, 0}, 0, {-16, -16}, {0, 0, 0, 0} }},
	{{ {35, 35, 0}, 0, {-16, -16}, {0, 0, 0, 0} }},
	{{ {35, 35, 0}, 0, {-16, -16}, {0, 0, 0, 0} }},
	{{ {35, -35, 0}, 0, {-16, -16}, {0, 0, 0, 0} }},
};

Vtx l6_2d_red_2DRedCoin_mesh_layer_4_vtx_0[4] = {
	{{ {-35, -35, 0}, 0, {0, 512}, {255, 255, 255, 126} }},
	{{ {35, -35, 0}, 0, {512, 512}, {255, 255, 255, 126} }},
	{{ {35, 35, 0}, 0, {512, 0}, {255, 255, 255, 126} }},
	{{ {-35, 35, 0}, 0, {0, 0}, {255, 255, 255, 126} }},
};

Gfx l6_2d_red_2DRedCoin_mesh_layer_4_tri_0[] = {
	gsSPVertex(l6_2d_red_2DRedCoin_mesh_layer_4_vtx_0 + 0, 4, 0),
	gsSP2Triangles(0, 1, 2, 0, 0, 2, 3, 0),
	gsSPEndDisplayList(),
};


Gfx mat_l6_2d_red__2DRed[] = {
	gsDPPipeSync(),
	gsDPSetCombineLERP(TEXEL0, 0, SHADE, 0, 0, 0, 0, TEXEL0, TEXEL0, 0, SHADE, 0, 0, 0, 0, TEXEL0),
	gsSPGeometryMode(G_LIGHTING, 0),
	gsDPSetTextureFilter(G_TF_POINT),
	gsSPTexture(65535, 65535, 0, 0, 1),
	gsDPSetTextureLUT(G_TT_RGBA16),
	gsDPSetTextureImage(G_IM_FMT_RGBA, G_IM_SIZ_16b, 1, l6_2d_red__2d_red_coin_ci4_pal_rgba16),
	gsDPSetTile(0, 0, 0, 256, 7, 0, G_TX_WRAP | G_TX_NOMIRROR, 0, 0, G_TX_WRAP | G_TX_NOMIRROR, 0, 0),
	gsDPLoadTLUTCmd(7, 15),
	gsDPLoadSync(),
	gsDPSetTextureImage(G_IM_FMT_CI, G_IM_SIZ_16b, 1, l6_2d_red__2d_red_coin_ci4),
	gsDPSetTile(G_IM_FMT_CI, G_IM_SIZ_16b, 0, 0, 7, 0, G_TX_WRAP | G_TX_NOMIRROR, 4, 0, G_TX_WRAP | G_TX_NOMIRROR, 4, 0),
	gsDPLoadBlock(7, 0, 0, 63, 2048),
	gsDPSetTile(G_IM_FMT_CI, G_IM_SIZ_4b, 1, 0, 0, 0, G_TX_WRAP | G_TX_NOMIRROR, 4, 0, G_TX_WRAP | G_TX_NOMIRROR, 4, 0),
	gsDPSetTileSize(0, 0, 0, 60, 60),
	gsSPEndDisplayList(),
};

Gfx mat_revert_l6_2d_red__2DRed[] = {
	gsDPPipeSync(),
	gsSPGeometryMode(0, G_LIGHTING),
	gsDPSetTextureFilter(G_TF_BILERP),
	gsDPSetTextureLUT(G_TT_NONE),
	gsSPEndDisplayList(),
};

Gfx l6_2d_red_2DRedCoin_mesh_layer_4[] = {
	gsSPClearGeometryMode(G_LIGHTING),
	gsSPVertex(l6_2d_red_2DRedCoin_mesh_layer_4_vtx_cull + 0, 8, 0),
	gsSPSetGeometryMode(G_LIGHTING),
	gsSPCullDisplayList(0, 7),
	gsSPDisplayList(mat_l6_2d_red__2DRed),
	gsSPDisplayList(l6_2d_red_2DRedCoin_mesh_layer_4_tri_0),
	gsSPDisplayList(mat_revert_l6_2d_red__2DRed),
	gsSPEndDisplayList(),
};

Gfx l6_2d_red_material_revert_render_settings[] = {
	gsDPPipeSync(),
	gsSPSetGeometryMode(G_LIGHTING),
	gsSPClearGeometryMode(G_TEXTURE_GEN),
	gsDPSetCombineLERP(0, 0, 0, SHADE, 0, 0, 0, ENVIRONMENT, 0, 0, 0, SHADE, 0, 0, 0, ENVIRONMENT),
	gsSPTexture(65535, 65535, 0, 0, 0),
	gsDPSetEnvColor(255, 255, 255, 255),
	gsDPSetAlphaCompare(G_AC_NONE),
	gsSPEndDisplayList(),
};

