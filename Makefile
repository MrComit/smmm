...
ifeq ($(COMPILER),gcc)
  CFLAGS := -mdivide-breaks -march=vr4300 -mfix4300 -mabi=32 -mno-shared -G 0 -mhard-float -fno-stack-protector -fno-common -fno-zero-initialized-in-bss -I include -I src/ -I $(BUILD_DIR)/include -fno-PIC -mno-abicalls -fno-strict-aliasing -fno-inline-functions -ffreestanding -fwrapv -Wall -Wextra $(COMMON_CFLAGS)
endif
...
ALL_DIRS := $(BUILD_DIR) $(addprefix $(BUILD_DIR)/,$(SRC_DIRS) $(ASM_DIRS) $(BIN_DIRS) $(TEXTURE_DIRS) $(TEXT_DIRS) $(SOUND_SAMPLE_DIRS) $(addprefix levels/,$(LEVEL_DIRS)) include) $(YAY0_DIR) $(addprefix $(YAY0_DIR)/,$(VERSION)) $(SOUND_BIN_DIR) $(SOUND_BIN_DIR)/sequences/$(VERSION) $(RSP_DIRS)
...
$(BUILD_DIR)/%.szp.o: $(BUILD_DIR)/%.szp.s
	@$(PRINT) "$(GREEN)Assembling ASM source file:  $(BLUE)$@ $(NO_COL)\n"
	@$(AS) $(ASFLAGS) -o $@ $<

$(BUILD_DIR)/%.szp.s: $(BUILD_DIR)/%.szp
	@$(PRINT) "$(GREEN)Dumping $(YELLOW)$< $(GREEN)to:  $(BLUE)$@ $(NO_COL)\n"
	@printf ".section .data\n\n.incbin \"$<\"\n" > $@
...
$(BUILD_DIR)/%.o: %.c
	...
	@$(CC) -c $(CFLAGS) -MMD -MF $(BUILD_DIR)/$*.d  -o $@ $<
...
$(ELF): $(O_FILES) $(YAY0_OBJ_FILES) $(SOUND_OBJ_FILES) $(SEG_FILES) $(BUILD_DIR)/$(LD_SCRIPT) undefined_syms.txt lib/libhvqm2.a
	@$(PRINT) "$(GREEN)Linking elf file:  $(BLUE)$@ $(NO_COL)\n"
	@$(LD) -L $(BUILD_DIR) $(LDFLAGS) -o $@ $(O_FILES)$(LIBS) -L $(N64_LIBS_DIR) -lultra_rom -lhvqm2
