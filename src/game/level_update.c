...
s16 level_trigger_warp(struct MarioState *m, s32 warpOp) {
    ...
    if (sDelayedWarpOp == WARP_OP_NONE) {
        ...
        switch (warpOp) {
            ...
            case WARP_OP_WARP_FLOOR_OBJECT:
                if (m->floor->type != SURFACE_DEATH_PLANE && warpOp != WARP_OP_WARP_FLOOR_OBJECT) {
...
void update_hud_values(void) {
    if (gCurrCreditsEntry == NULL && gCurrDemoInput == NULL) {
        ...
        if (gMarioState->action != ACT_READING_NPC_DIALOG && gMarioState->action != ACT_READING_AUTOMATIC_DIALOG
            && gMarioState->action != ACT_IN_CANNON && gMarioState->action & ACT_FLAG_STATIONARY && gCamera->comitCutscene == 0) {
            if (sTimer2++ > 45) {
...
s32 play_mode_paused(void) {
    ...
    } else if (gMenuOptSelectIndex == MENU_OPT_MAP) {
