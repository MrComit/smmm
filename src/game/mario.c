...
static void set_mario_y_vel_based_on_fspeed(struct MarioState *m, f32 initialVelY, f32 multiplier) {
    ...
    struct Object *obj = m->marioObj->platform;
    ...
    if (obj != NULL && obj_has_behavior(obj, bhvObservatorySpinningPlat)) {
        if (obj->oVelY > 0.0f) {
            m->vel[1] += obj->oVelY;
            m->pos[1] += obj->oVelY;
        }
...
void debug_print_speed_action_normal(struct MarioState *m) {
    ...
    if (gShowDebugText) {
        ...
        print_text_fmt_int(210, 88, "ANG %d", (atan2s(floor_nY, steepness) * 180.0f) / 32768.0, 0);

        print_text_fmt_int(210, 72, "SPD %d", m->forwardVel, 0);

        // STA short for "status," the official action name via SMS map.
        print_text_fmt_int(210, 56, "STA %x", (m->action & ACT_ID_MASK), 0);
    }
}
    ...
    if (m->intendedMag > 0.0f) {
        ...
        if (m->action == ACT_TIGHT_ROPE || m->action == ACT_TIGHT_ROPE_WALKING) {
            if ((obj = m->marioObj->platform) != NULL) {
                dist = absi(m->intendedYaw - obj->oFaceAngleYaw);
                if (dist < 0x4000) {
                    if (dist >= 0x2000) {
                        m->intendedMag = 0.0f;
...
void update_treadmills(struct MarioState *m) {
    f32 displace;
    if ((gPowerOn || gCurrLevelNum == LEVEL_TTC) && m->floor->type == SURFACE_TREADMILL && m->pos[1] <= m->floorHeight) {
