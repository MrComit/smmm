...
u32 interact_coin(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    if (gHudDisplay.flags & HUD_DISPLAY_FLAG_BOO) {
        m->numBooCoins += o->oDamageOrCoinValue;
    } else {
...
u32 interact_door(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    u32 saveFlags = save_file_get_keys(1);
    u32 doorAction;
    ...
    if (o->oInteractionSubtype & INT_SUBTYPE_LOCKED_DOOR) {
        if (!(keyCount & (1 << o->oBehParams2ndByte))) {
            ...
            sDisplayingDoorText = TRUE;
            return FALSE;
        } else if (!(saveFlags & (1 << o->oBehParams2ndByte))) {
            m->interactObj = o;
            m->usedObj = o;
...
u32 interact_pole(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (actionId >= 0x080 && actionId < 0x0A0) {
        if ((!(m->prevAction & ACT_FLAG_ON_POLE) || (m->actionTimer > 10 && m->action == ACT_TRIPLE_JUMP))|| m->usedObj != o) {
            ...
            if (o->oInteractionSubtype & INT_SUBTYPE_HORIZONTAL) {
                if (absi(m->faceAngle[1] - o->oFaceAngleYaw) < 0x4000)
                    m->faceAngle[1] = o->oFaceAngleYaw;
                else
                    m->faceAngle[1] = o->oFaceAngleYaw + 0x8000;
            }
...
void check_kick_or_punch_wall(struct MarioState *m) {
    ...
    if (m->flags & (MARIO_PUNCHING | MARIO_KICKING | MARIO_TRIPPING)) {
        ...
        if (wall != NULL) {
            ...
            if (wall->object != NULL) {
                wall->object->oFlags |= OBJ_FLAG_KICKED_OR_PUNCHED;
            }
        }
    }
}
...
void check_death_barrier(struct MarioState *m) {
    ...
    if (sDeathFloorTimer) {
        ...
        if (m->health < 0x300 || gCurrCourseNum >= 11) {
    ...
    if (m->floor != NULL) {
        ...
        switch (floorType) {
            ...
            case SURFACE_WARP_HEIGHT_HIGHER:
                if (floorType == SURFACE_WARP_HEIGHT_LOWER) {
                    if (m->pos[1] - m->floorHeight > 200.0f) {
                        break;
                    }
                } else {
                    if (m->pos[1] - m->floorHeight < 600.0f) {
                        break;
                    }
                }
            case SURFACE_WARP:
