...
s32 act_in_cannon(struct MarioState *m) {
    ...
    switch (m->actionState) {
        case 0:
            ...
            m->pos[0] = m->usedObj->oPosX;
            m->pos[1] = m->usedObj->oPosY + 450.0f;
            m->pos[2] = m->usedObj->oPosZ;
        ...
        case 2:
            if (absf(m->controller->stickY) > 16.0f) {
                if (absf(m->controller->stickY) > 48.0f) {
                    m->faceAngle[0] -= (s16)(64.0f * 5.0f * signum_positive(m->controller->stickY)); //vanilla 10.0f
                } else {
                    m->faceAngle[0] -= (s16)(m->controller->stickY * 5.0f); //vanilla 10.0f
                }
            }
            if (absf(m->controller->stickX) > 16.0f) {
                if (absf(m->controller->stickX) > 48.0f) {
                    marioObj->oMarioCannonInputYaw -= (s16)(64.0f * 7.0f * signum_positive(m->controller->stickX)); //vanilla 10.0f
                } else {
            ...
            if (marioObj->oMarioCannonInputYaw > 0x2000) {
                marioObj->oMarioCannonInputYaw = 0x2000;
            }
            if (marioObj->oMarioCannonInputYaw < -0x2000) {
                marioObj->oMarioCannonInputYaw = -0x2000;
            }
            ...
            if (++m->actionTimer >= 8 && m->input & INPUT_A_PRESSED) {
                ...
                if (gCurrLevelNum != LEVEL_SL) {
                    ...
                    obj = spawn_object(marioObj, MODEL_BOWLING_BALL, bhvNewCannonBalls);
                }
            ...
            if ((cur_obj_nearest_object_with_behavior(bhvShootingGalleryHandler) == NULL) && 
                !(gCurrLevelNum == LEVEL_SL && (obj = cur_obj_nearest_object_with_behavior(bhvNewShootingGalleryHandler)) != NULL 
                && obj->oAction != 2)) {
                ...
                if (gCurrLevelNum != LEVEL_SL) {
                    ...
                    obj = spawn_object(marioObj, MODEL_DL_CANNON_LID, bhvNewCannonLid);
                }
                ...
                if (gCurrLevelNum == LEVEL_SL) {
                    obj = spawn_object(m->usedObj, MODEL_CANNON_BASE, bhvCannon);
                    obj->oBehParams2ndByte = 0x40;
