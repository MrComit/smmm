...
void fixed_cam_presets(struct Camera *c) {
    ...
    switch (c->comitCutscene) {
        ...
        case 13:
            ...
            break;
        case 14:
            vec3f_set(c->pos, m->pos[0] + 350.0f, m->pos[1] + 200.0f, m->pos[2] + 1000.0f);
            vec3f_set(c->focus, 2149.0f, 13087.0f, -17810.0f);
            vec3f_get_dist_and_angle(c->focus, c->pos, &pos[0], &pitch, &yaw);
            c->yaw = c->nextYaw = yaw;
            break;
        case 15:
        ...
        case 17:
            ...
            break;
        case 18:
            vec3f_set(c->pos, 8900.0f, -1200.0f, m->pos[2] - 1750.0f);
        ...
        case 20:
            ...
            pos[0] = obj->oPosX + 3000.0f * sins(obj->oFaceAngleYaw);
            pos[1] = obj->oPosY + 300.0f;
            pos[2] = obj->oPosZ + 3000.0f * coss(obj->oFaceAngleYaw);

            // vec3f_copy(c->pos, gComitCutscenePosVec);
            vec3f_copy(c->pos, pos);
            vec3f_set(c->focus, obj->oPosX, obj->oPosY, obj->oPosZ);
            c->yaw = c->nextYaw = obj->oFaceAngleYaw + 0x8000;//DEGREES(0);
            break;
        case 21:
            vec3f_set(c->pos, -12715.0f, 8900.0f, m->pos[2]);
            vec3f_set(c->focus, -9215.0f, 8900.0f, m->pos[2]);
            c->yaw = c->nextYaw = DEGREES(270);
            break;
        case 22:
            vec3f_set(c->pos, m->pos[0], 8900.0f, 10008.0f);
            vec3f_set(c->focus, m->pos[0], 8900.0f, 6508.0f);
            c->yaw = c->nextYaw = DEGREES(0);
            break;
        case 23:
            vec3f_set(c->pos, m->pos[0], 8900.0f, 7258.0f);
            vec3f_set(c->focus, m->pos[0], 8900.0f, 10758.0f);
            c->yaw = c->nextYaw = DEGREES(180);
            break;
        case 24:
            c->pos[1] = gComitCutscenePosVec[1];
            c->comitCutscene = 0;
            break;
        case 25:
            vec3f_set(c->pos, m->pos[0] + 11000.0f, 4500.0f, m->pos[2] - 9000.0f);
        ...
        case 27:
            ...
            break;
        case 28:
            vec3f_set(c->pos, m->pos[0] + 2300.0f, 2900.0f, m->pos[2] + 14000.0f); // was +5500.0f, 8800.0f, +7200.0f
            vec3f_set(c->focus, m->pos[0] + 300.0f, 9000.0f, m->pos[2]);
            c->yaw = c->nextYaw = 0x1162;
            break;
        case 29:
            if (gComitCutsceneObject == NULL) {
        ...
        case 34:
            ...
            vec3f_copy(c->focus, &gComitCutsceneObject->oPosX);
            vec3f_set(c->pos, gComitCutsceneObject->oPosX + 700.0f, gComitCutsceneObject->oPosY + 800.0f, 
                    gComitCutsceneObject->oPosZ - 1800.0f);
...
f32 gCam2dHeight = 0;
s32 gCam2dHeightTimer = 0;

void set_2dcam_height(struct Camera *c) {
    struct MarioState *m = gMarioState;
    f32 curHeight = m->pos[1] - c->pos[1];
    if (gCam2dHeight <= m->pos[1] + 300.0f) {
        if (gCam2dHeightTimer++ > 45) {
            gCam2dHeight = approach_f32_symmetric(gCam2dHeight, m->pos[1] + 300.0f, 20.0f);
            gCam2dHeightTimer = 46;
    ...
    if (curHeight > MAX_HEIGHT) {
        gCam2dHeight = m->pos[1] + 300.0f;
        gCam2dHeightTimer = 0;
    } else if (curHeight < MIN_HEIGHT) {
        gCam2dHeight = m->pos[1] - 100.0f;
    }
    c->pos[1] = approach_f32_symmetric(c->pos[1], gCam2dHeight, 60.0f);
}
...
void mode_8_directions_camera_2d(struct Camera *c) {
    ...
    switch (c->comit2dcam) {
        ...
        case 2:
            ...
            m->pos[0] = approach_f32_symmetric(m->pos[0], 16111.0f, 80.0f);
            cam_controls_2d(c);
            update_8_directions_camera(c, c->focus, pos);
            c->pos[2] = pos[2];
            c->focus[2] = m->pos[2];
        ...
        case 4:
            ...
            c->pos[2] = 4100.0f + gDepthOffset2d;
            c->yaw = c->nextYaw = s8DirModeBaseYaw;
            pos[1] += 500.0f;
            approach_camera_height(c, pos[1], ABS(c->pos[1] - (pos[1] + 300.0f)) / 20);
            if (gPlayer1Controller->buttonPressed & R_TRIG && c->cutscene == 0) {
        ...
        case 6:
            ...
            c->pos[2] = 6600.0f + gDepthOffset2d;
            c->yaw = c->nextYaw = s8DirModeBaseYaw;
            pos[1] += 500.0f;
            approach_camera_height(c, pos[1], ABS(c->pos[1] - (pos[1] + 300.0f)) / 20);
            if (gPlayer1Controller->buttonPressed & R_TRIG && c->cutscene == 0) {
...
void update_camera(struct Camera *c) {
    ...
    //} else {
    if (gGlobalMarioRoom != 11 && gMarioState->action != ACT_IN_CANNON && (c->mode != CAMERA_MODE_C_UP || c->comitCutscene != 0)) {
...
u8 sZoomOutAreaMasks[] = {
	...
	ZOOMOUT_AREA_MASK(1, 1, 0, 0, 1, 0, 0, 0), // SSL            | BOB
	ZOOMOUT_AREA_MASK(0, 0, 0, 0, 1, 0, 0, 0), // SL             | WDW
	ZOOMOUT_AREA_MASK(1, 1, 0, 0, 1, 0, 0, 0), // JRB            | THI
