...
static void level_cmd_clear_level(void) {
    ...
    clear_areas();
    main_pool_pop_state();
    // the game does a push on level load and a pop on level unload
    // we need to add another push to store state after the level has been loaded, so one more pop is needed
    main_pool_pop_state();
    sCurrentCmd = CMD_NEXT;
}
    ...
    if (sCurrAreaIndex != -1) {
        if (gAreas[sCurrAreaIndex].instantWarps == NULL) {
            gAreas[sCurrAreaIndex].instantWarps =
                alloc_only_pool_alloc(sLevelPool, 8 * sizeof(struct InstantWarp));
...
static void level_cmd_create_whirlpool(void) {
    ...
    s32 beatBowser2 = 0;//(save_file_get_flags() & (SAVE_FLAG_HAVE_KEY_2 | SAVE_FLAG_UNLOCKED_UPSTAIRS_DOOR)) != 0;
